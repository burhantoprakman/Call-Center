...
class TodayVC : UIViewController ,IndicatorInfoProvider, UITableViewDelegate , UITableViewDataSource,ActivityIndicatorPresenter  {
 
    
    @IBOutlet weak var bTodayTableView: UITableView!
    var bTodayResult = [BirthdayPojo]()
    var autoid : String = ""
    ...
    func tebrikClicked( sender: UIButton) {
        likedPersonId = bTodayResult[sender.tag].tebrikButton
        showActivityIndicator()
        tebrikLiked()
        }
    func instaClicked( sender: UIButton) {
        likedPersonId = bTodayResult[sender.tag].instaString
    ...
    func faceClicked( sender: UIButton) {
        likedPersonId = bTodayResult[sender.tag].faceString
    ...
    func getTodayBirthday(){
        ...
        let url = URL(string: "https://ekolife.ekoccs.com/api/General/GetBirthDays?req=1")!
        let request = URLRequest(url: url)
        let config = URLSessionConfiguration.default
        let together = "\(autoid) \(firebaseToken)"
        config.httpAdditionalHeaders = ["Authorization" : together]
        ...
        let task: URLSessionDataTask = session.dataTask(with: request) { (data, response, error)  -> Void in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    self.hideActivityIndicator()
                    self.bTodayResult = resultArray
                    self.bTodayTableView.reloadData()
                }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell : BirthdayTodayCell = bTodayTableView.dequeueReusableCell(withIdentifier: "birthdaytodaycell", for: indexPath) as! BirthdayTodayCell
        ...
        let goCell  =  bTodayResult[indexPath.row]
        cell.profImage.image = #imageLiteral(resourceName: "Kişiler.png")
        ...
        cell.insstaButton.tag = indexPath.row
        cell.insstaButton.addTarget(self, action: #selector(instaClicked(sender:)), for: .touchUpInside )
